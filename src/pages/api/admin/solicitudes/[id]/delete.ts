...
export const POST: APIRoute = async ({ params, cookies, redirect, request }) => {
  ...
  const { id } = params;
  if (!id) return redirect(flashRedirect('/admin/solicitudes', 'ID inválido.', 'error'), 303);

  const sb = supabaseAdmin();
  const { error } = await sb.from('solicitudes_socio').delete().eq('id', id);
  if (error) return redirect(flashRedirect(`/admin/solicitudes/${id}`, error.message, 'error'), 303);
  return redirect(flashRedirect('/admin/solicitudes', 'Solicitud eliminada.'), 303);
};
